...
namespace Drupal\crudform\Form;
 ...
 * Class CrudformForm.
 *
 * @package Drupal\crudform\Form
 */
class CrudformForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if (isset($_GET['num'])) {
          ...
          $query->update('crudform')
              ->fields($field)
              ->condition('id', $_GET['num'])
              ->execute();
      \Drupal::messenger()->addMessage($this->t("succesfully updated"));
          $form_state->setRedirect('crudform.display_table_controller_display');
       ...
       else
       {
           ...
           $query = \Drupal::database();
           $query ->insert('crudform')
               ->fields($field)
               ->execute();
         \Drupal::messenger()->addMessage($this->t("succesfully saved"));

           $response = new RedirectResponse("/crudform/hello/table");
           $response->send();
       }
     }
